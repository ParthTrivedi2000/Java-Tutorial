...
/* Very good thinking / observation
Approach:-
## Intuition ##
The problem is to find the minimum difference between any two time points in a list of times given in "HH:MM" format.
To solve this problem, you need to consider that the times wrap around from "23:59" to "00:00" (effectively forming
a circular timeline).

## Approach ##
- Convert Time to Minutes:
    Convert each time point from "HH:MM" format to the number of minutes since midnight. This simplifies the comparison
    of time differences to just integer subtraction.

- Sort the Time Points:
    After converting times to minutes, sort these minutes. The reason for sorting is that the minimum difference
    between any two times in a circular timeline will be between adjacent elements in the sorted list.

- Calculate Differences:
    Compute the difference between each pair of adjacent elements in the sorted list. This step finds the minimum
    difference between consecutive times.

- Consider Circular Nature:
    Since the times are on a 24-hour circular clock, you must also consider the difference between the last time and
    the first time (after wrapping around midnight).
...
TimePoint	Hours (H)	Minutes (M)	Total Minutes (H * 60 + M)
...
Finding Minimum Difference: O(n). After sorting, you need to find the minimum difference by checking adjacent elements
in the sorted list and the circular wrap-around difference.
