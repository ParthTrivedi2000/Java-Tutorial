...
each cookie j has a size s[j]. If s[j] >= g[i], we can assign the cookie j to the child i, and the child i will be content. Your
...
public class G01AssignCookies {
    public int findContentChildren(int[] g, int[] s) {
        // Approach:-
        // So since we want to distribute cookies such that, childrens are maximised.
        // So suppose I am having cookies array as s and greediness of child array as g.
        // Now for distributing the cookies, think like I want to give cooki to child such
        // that just his greediness is fulfilled. So for that greedily I will try to give cooki as much
        // close to his greedines. for example, if child has greedyness of 3, then and I can have
        // option to give 1,5,7,3,4 then I instead of giving random from it, I will try to give
        // 3 so that other higher valued cookies can be ustilised for next childers whom might
        // have more greediness.

        // So to distribute such as maximum childern fulfilled, I need to sort the child greediness
        ...
        // wherever my i is there/or stopped, utne childerens were feeded, so return it.
        return i;
    }
}
