...
Your Task:
You don't need to read input or print anything. Your task is to complete the function solve() which takes bt[] as
input parameter and returns the average waiting time of all the processes.
...
public class G03ShortestJobFirst {
    ...
    static int solve(int bt[]) {
        ...
        Arrays.sort(bt);
        int waitingTime=0;
        int time=0; // this is the time process takes to execute i.e. burst time
        for(int elem:bt){
            waitingTime+=time;
            time+=elem;
        }
