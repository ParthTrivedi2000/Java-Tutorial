...
- So we will create a seperate helper class (which basically act as DS for us to store all the attributes of the
...
/*
So above if you see, unless you create bulder class and set attribute values, your Student object is not in
...
attributes in seperate class as DS).
So let's do it.



So in above code if you see, as a new developer's perspective, untill line no 15 there is no code for student
class howewer we are working for Student class. (It might happen ki you feel it's easily understandable, but
think about 15-20 attributes, set over here manually in the code, so you must also get consfused by seeing
...
remember about the builder clsas nd need to create object of builder to just some other class (here Student).
